...
"""
#manipulated_df.to_csv('Yeni_All_manipulated.txt', index=False, sep='\t')  #Checkpoint

# Further manipulation of the data and filling mising values
import math


df_file = 'Yeni_All_manipulated.txt'
...
def rounder_func(str):
	'''
	if it's convertible, converts str to float and rounds to 3 decimal places
	:param str: string values
	:return: float and rounded values
	'''
	try:
		flt = float(str).__round__(3)
		return flt
...
rounded_new_df.to_csv('Yeni_Filled_cleaned_coronadf.txt', index=False, sep='\t')            #Checkpoint for filled_cleaned up data
...
cigarated_df.to_csv('Yeni_smoker_added_coronadf.txt', index=False, sep='\t')			#Checkpoint    smokers